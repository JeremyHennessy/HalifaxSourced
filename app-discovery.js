...
function mapResultRow(restaurant) {
  return `<article class="map-result-row"><div class="map-thumb media-${mediaTone(restaurant)}${permittedImageClass(restaurant)}">${mediaImageMarkup(restaurant)}</div><div><strong>${escapeHtml(restaurant.name)}</strong><span>${escapeHtml(primaryCuisine(restaurant))} · ${escapeHtml(restaurant.neighborhood || "Halifax")}</span><small>${consumerTags(restaurant).slice(0, 2).map(escapeHtml).join(" · ")}</small></div><a href="#restaurant/${encodeURIComponent(restaurant.id)}">View</a><button class="row-save ${state.saved.has(restaurant.id) ? "is-saved" : ""}" data-save-id="${escapeHtml(restaurant.id)}" aria-label="Save ${escapeHtml(restaurant.name)}">${state.saved.has(restaurant.id) ? "♥" : "♡"}</button></article>`;
}
