  ...
  appView.innerHTML = `
    ...
    <section class="page-shell detail-layout">
      <div class="detail-main">
        ...
        ${officialUpdates.length ? `<section id="detailUpdates" class="detail-section"><div class="section-heading no-top"><div><h2>Latest official updates</h2><p>Recent posts observed from restaurant-owned feeds or authenticated official social APIs. Summaries and categories are generated from bounded source excerpts; open the original for full terms.</p></div></div><div class="official-update-grid">${officialUpdates.slice(0, 12).map(officialUpdateCard).join("")}</div></section>` : ""}
...
function officialUpdateCard(update) {
  ...
  const review = update.reviewState && update.reviewState !== "source_signal" && update.reviewState !== "api_observed" ? `<em>${escapeHtml(update.reviewState.replace(/_/g, " "))}</em>` : "";
  return `<a class="official-update-card${mediaUrl ? " has-media" : ""}" href="${escapeHtml(url)}" target="_blank" rel="noreferrer">${media}<span>${escapeHtml(platform)}</span><strong>${escapeHtml(update.title || `${platform} update`)}</strong>${summary ? `<p>${escapeHtml(summary)}</p>` : ""}${chips}<small>${escapeHtml(date)} · ${escapeHtml(update.primaryCategoryLabel || "Open original")} ↗</small>${review}</a>`;
}
