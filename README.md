...
- normalized recent-post intelligence for official feeds, official page intelligence, and permitted social APIs
- bounded official page intelligence for menus, specials, happy hours, patios, openings, brunch, live music, and events
...
Website feed results are stored in `data/website-feed-signals.js` with bounded excerpts rather than long feed bodies. Official page intelligence is stored in `data/website-page-intelligence.js` with bounded titles/excerpts, source URLs, candidate images, and matched categories from restaurant-owned pages. `scripts/build-recent-social-posts.mjs` normalizes official feed, official page, and Meta API updates into categories, summaries, matched terms, recency state, and review state. Feed/social signals remain historical evidence, but unstructured items set current consumer flags only when they have a parseable publication timestamp no more than 60 days old.
...
Existing import scripts remain under `scripts/`. See `docs/ingestion-compliance.md`, `docs/media-provenance.md`, `docs/thumbnail-candidate-pipeline.md`, `docs/recent-post-intelligence.md`, `docs/official-page-intelligence.md`, `docs/structured-events.md`, `docs/verified-source-pages.md`, `docs/social-and-first-party-ingestion.md`, `data/social-platform-registry.json`, and `data/source-contract.json` before extending collection.
