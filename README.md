...
First-party source discovery is stored separately in `data/first-party-sources.js`. Restaurant-owned websites are used to establish official Facebook/Instagram profile associations and discover same-site RSS/Atom feeds. Website feed results are stored in `data/website-feed-signals.js` without retaining long feed bodies.
...
The `Source Expansion Preview` workflow refreshes website signals, verified source pages, structured events, restaurant-owned feeds, first-party Facebook/Instagram associations, and permitted Meta API signals. It runs daily and can be started manually. Generated output is uploaded for review rather than automatically replacing production data.
